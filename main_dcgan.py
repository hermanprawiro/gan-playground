...
def main(args):
    ...
    netG = dcgan.Generator(latent_dim=args.latent_dim, out_dim=args.image_ch).to(device)
    netD = dcgan.Discriminator(in_dim=args.image_ch).to(device)
    netG.apply(weights_init)
    netD.apply(weights_init)

    optG = torch.optim.Adam(netG.parameters(), lr=args.learning_rate, betas=(args.beta1, args.beta2))
    optD = torch.optim.Adam(netD.parameters(), lr=args.learning_rate, betas=(args.beta1, args.beta2))
        ...
        for i, (inputs, _) in enumerate(dataloader):
            ...
            if i % 50 == 0:
                print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f D(x): %.4f D(G(z)): %.4f / %.4f'
                    % (epoch + 1, args.n_epochs, i, len(dataloader), lossD.item() / 2, lossG.item(), Dx, Dgz1, Dgz2))
